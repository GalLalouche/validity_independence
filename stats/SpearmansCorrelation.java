...
final class SpearmansCorrelation {
	...
	/**
	 * Returns a {@link PearsonsCorrelation} instance constructed from the ranked
	 * input data. That is,
	 ...
	 */
	public Pair<Double, Double> correlation(final double[] xArray,
			final double[] yArray) {
		if (xArray.length != yArray.length) {
			...
			if (rankingAlgorithm instanceof NaturalRanking
					&& NaNStrategy.REMOVED == ((NaturalRanking) rankingAlgorithm)
							.getNanStrategy()) {
	...
	private RealMatrix rankTransform(final RealMatrix matrix) {
		...
		if (rankingAlgorithm instanceof NaturalRanking
			...
			if (!nanPositions.isEmpty()) {
				transformed = new BlockRealMatrix(matrix.getRowDimension()
						- nanPositions.size(), matrix.getColumnDimension());
				for (int i = 0; i < transformed.getColumnDimension(); i++) {
					transformed.setColumn(i,
							removeValues(matrix.getColumn(i), nanPositions));
				}
			}
		}
